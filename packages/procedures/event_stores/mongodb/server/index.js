...
const eventStore = async ({ schema, indexes, secretFn }) => {
  ...
  _eventStore = await deps.db.store({
...
const snapshotStore = async ({ schema, indexes }) => {
  ...
  _snapshotStore = await deps.db.store({
    ...
    schema: {
      ...
      headers: {
        ...
        eSize: { [typeKey]: Number, required: true },
        _id: false,
      },
      context: { [typeKey]: Object },
      state: schema,
...
const countsStore = async () => {
  ...
  _countsStore = await deps.db.store({
...
const blockchainStore = async () => {
  ...
  _blockchainStore = await deps.db.store({
    ...
    schema: {
      ...
      headers: {
        ...
        eSize: { [typeKey]: Number, required: true },
        sSize: { [typeKey]: Number, required: true },
        tSize: { [typeKey]: Number, required: true },
        domain: { [typeKey]: String, required: true },
        service: { [typeKey]: String, required: true },
        network: { [typeKey]: String, required: true },
