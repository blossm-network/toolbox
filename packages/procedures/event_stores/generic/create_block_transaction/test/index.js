...
describe("Event store create block transaction", () => {
  ...
  it("should call with the correct params", async () => {
    ...
    expect(hashFake.getCall(6)).to.have.been.calledWith({
      ...
      eSize: Buffer.byteLength(Buffer.from(encodedEventPairs)),
    });
    ...
    expect(saveSnapshotFnFake).to.have.been.calledOnceWith({
      snapshot: {
        ...
        state: aggregateState,
        groups: aggregateGroups,
        events: Buffer.from(encodedEventPairs),
        txIds,
      },
      transaction,
    });
    ...
    expect(hashFake.getCall(10)).to.have.been.calledWith({
      ...
      eSize: Buffer.byteLength(Buffer.from(encodedAllEventPairs)),
      sSize: Buffer.byteLength(Buffer.from(encodedSnapshotPairs)),
      tSize: Buffer.byteLength(Buffer.from(encodedTxPairs)),
      network,
      service,
      domain,
    ...
    expect(saveBlockFnFake).to.have.been.calledOnceWith({
      block: {
        ...
        events: Buffer.from(encodedAllEventPairs),
        snapshots: Buffer.from(encodedSnapshotPairs),
        txs: Buffer.from(encodedTxPairs),
      },
      transaction,
    });
  });
  ...
  it("should call with the correct params with no previous block", async () => {
    ...
    expect(hashFake.getCall(1)).to.have.been.calledWith({
      ...
      eSize: Buffer.byteLength(Buffer.from(encodedEmptyPairs)),
      sSize: Buffer.byteLength(Buffer.from(encodedEmptyPairs)),
      tSize: Buffer.byteLength(Buffer.from(encodedEmptyPairs)),
      network,
      service,
      domain,
      key: Buffer.from(publicKey).toString("base64"),
    ...
    expect(saveBlockFnFake).to.have.been.calledOnceWith({
      block: {
        ...
        events: Buffer.from(Buffer.from(encodedEmptyPairs)),
        snapshots: Buffer.from(Buffer.from(encodedEmptyPairs)),
        txs: Buffer.from(Buffer.from(encodedEmptyPairs)),
      },
      transaction,
    });
  });
