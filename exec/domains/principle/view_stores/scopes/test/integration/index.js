...
describe("View store integration tests", () => {
  ...
  it("should return successfully", async () => {
    ...
    expect(parsedBody1.scopes.length).to.equal(1);
    expect(parsedBody1.scopes[0]).to.equal(scope);
    ...
    await request.stream(`${url}/stream`, data => {
      ...
      expect(parsedData.scopes.length).to.equal(1);
      expect(parsedData.scopes[0]).to.equal(scope);
    });
    ...
    expect(parsedBody4.scopes.length).to.equal(0);
  ...
  it("should return an error if incorrect params", async () => {
    const response = await request.post(url, { body: { scopes: { a: 1 } } });
